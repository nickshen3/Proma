...
import { X, ExternalLink, ChevronRight, MoreHorizontal, FolderSearch, Pencil, FolderInput, GitBranch, GitMerge, MessageSquarePlus, FileDiff, FileText, FolderOpen, Globe, MessageCircle, Brain, Split, Blocks, CalendarDays, ListTodo, Clock, ServerCog, SquareTerminal, Cpu, Terminal } from 'lucide-react'
    ...
    panePreviewId && panePreviewFile ? (
      ...
      <ProcessesPanel
        ...
        childSessions={[
          ...sessions
            .filter((item) => item.parentSessionId === sessionId && !!item.sourceDelegationId)
            .map((child) => ({ sessionId: child.id, label: getDelegationTabLabel(child.title) })),
          ...sideTemporaryAgents.map((branch) => ({
